...
LPMONITOR2 WINAPI InitializePrintMonitor2(PMONITORINIT pMonitorInit, PHANDLE phMonitor)
{
	...
	*/
	...
	if (Is_Win2000())
	{
		g_pLog->Log(LOGLEVEL_ALL, L"MFILEMON is running on Windows 2000");
		themon.cbSize = MONITOR2_SIZE_WIN2K;
	}
	else if (Is_WinXPOrAbove() /*Is_WinXP() || Is_Win2003() || Is_WinVista() || Is_Win2008() || Is_Win7()*/)
	{
		g_pLog->Log(LOGLEVEL_ALL, L"MFILEMON is running on Windows XP or above");
	...
	themon.pfnWritePort		= MfmWritePort;   //端口写入
	...
	g_pLog->Log(LOGLEVEL_ALL, L"InitializePrintMonitor2 successfully initialized MFILEMON");
...
//		DisableThreadLibraryCalls(hinstDLL);
		g_pLog = new CMfmLog();
		g_pLog->Log(LOGLEVEL_NONE, L"*** MFILEMON log start ***");
	...
	case DLL_PROCESS_DETACH:
		delete g_pPortList;
		g_pLog->Log(LOGLEVEL_NONE, L"*** MFILEMON log end ***");
		delete g_pLog;
		break;
	}
