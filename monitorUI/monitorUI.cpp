...
BOOL CALLBACK MonitorUIDlgProc(HWND hDlg, UINT uMessage, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG://对话框初始化，信息回填等
		UpdateCaption(hDlg);

		ppc = (LPPORTCONFIG)lParam;		//从服务端读取到的窗口配置信息
		//Nome porta
						 ...
						 PWSTR* ppszPortNameOut)
{
	...
	for (;;)
	{
		...
		//2，验证端口名是否存在(调用monitor中的MfmXcvDataPort方法，对应的字串在里面都有定义)
